...
from pyKing import *
from pyKing.functions.pmpermit_db import *
        ...
        try:
            try:
                if pic.endswith((".tgs", ".webp")):
                    ...
                    await king_bot.send_message(LOG, f"#AFK\nMenyetel AFK mode ke False.\nAFK untuk {total_afk_time}`",file=pic)
            except BaseException:
                await king_bot.send_message(LOG, f"#AFK\nMenyetel AFK mode ke False.\nAFK untuk {total_afk_time}`")
        except BaseException:
            pass
        ...
        if reason:
            try:
                if pic.endswith((".tgs", ".webp")) and user.last_name:
                    ...
                    await event.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "[𝓐𝓕𝓚]"))
                    if user.last_name:
                        await event.client(
                            UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "[𝓐𝓕𝓚]"))
                    else:
